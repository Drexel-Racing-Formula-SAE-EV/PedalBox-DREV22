...
void loop() {
    ...
    int left_sensor_percentage =
        100 - map(left_poten_value, left_poten_range.lower_bound,
                  left_poten_range.upper_bound, 0, 100);
    int right_sensor_percentage =
        100 - map(right_poten_value, right_poten_range.lower_bound,
                  right_poten_range.upper_bound, 0, 100);

    char buf[64];
    snprintf(buf, sizeof(buf), "[%d, %d] [%d, %d]\n", left_poten_value,
             right_poten_value, left_sensor_percentage,
             right_sensor_percentage);
    Serial.println(buf);
}
#if 0
